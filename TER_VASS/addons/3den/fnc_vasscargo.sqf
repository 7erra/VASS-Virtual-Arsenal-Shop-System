/*
	...
	Description:
		Internal function to set the cargo of a trader inside Eden.

	Parameter(s):
		0:	STRING - Mode
		1:	ARRAY - Arguments for the mode
		Optional:
...
#define SELF TER_VASS_fnc_vasscargo
...
switch _mode do {
	case "onLoad":{
		_params params ["_grp"];
		if (isNil {uiNamespace getVariable "TER_VASS_fnc_vasscargo"}) then {
			uiNamespace setVariable ["TER_VASS_fnc_vasscargo", TER_VASS_fnc_vasscargo];
		};
		//--- Init UI
	...
	case "keyCargoArray":{
		_params params ["_edCargoArray", "_key", "_shift", "_ctrl", "_alt"];
	...
	case "filterChanged":{
		_params params ["_toolFilter","_ind"];
	...
	case "keylnb":{
		_params params ["_grp", "_display", "_key", "_shift", "_ctrl", "_alt"];
	...
	case "validate":{
		_params params ["_btnValidate"];
		_grp = ctrlparentcontrolsgroup _btnValidate;
		_txtValidate = _grp controlsgroupctrl IDC_VASSCARGO_TXTVALIDATE;
		_edCargoArray = _grp controlsgroupctrl IDC_VASSCARGO_EDCARGOARRAY;
		//--- Go through array and compare format:
		_cargo = call compile ctrlText _edCargoArray;
		if !(_cargo isequaltype [] OR isNil "_cargo") exitwith {_txtValidate ctrlsetstructuredtext parsetext "<t color='#CC0000'>ERROR: Not an array</t>"};
		_message = "<t color='#00CC00'>Array is valid</t>";
		for "_i" from 0 to (count _cargo -1) step 3 do {
			...
			if !(_class isequaltype "") exitwith {_message = format ["<t color='#CC0000'>Class is not a string (Position: %1, %2)</t>",_i,str _class]};
			if (isnil "_price") exitwith {_message = format ["<t color='#CC0000'>(%1) Price not defined</t>",_class]};
			if !(_price isequaltype 0) exitwith {_message = format ["<t color='#CC0000'>%1: Price not a number (%2)</t>",_class,str _price]};
			if (isnil "_amount") exitwith {_message = format ["<t color='#CC0000'>(%1) Amount not defined</t>",_class]};
			if !(_amount isequaltypeany [0,true]) exitwith {_message = format ["<t color='#CC0000'>ERROR: %1: Amount not a number/boolean (%2)</t>",_class,str _amount]};
		};
		_txtValidate ctrlsetstructuredtext parsetext _message;
	};
	case "clear":{
		_params params ["_btnClear"];
	...
	case "priceChange":{
		_params params ["_edPrice", "_key", "_shift", "_ctrl", "_alt"];
	...
	case "presetChange":{
		_params params ["_comboPreset","_ind"];
	...
	case "attributeLoad":{
		_params params ["_grp","_value",["_from3den",false]];
		_edCargoArray = _grp controlsgroupctrl IDC_VASSCARGO_EDCARGOARRAY;
