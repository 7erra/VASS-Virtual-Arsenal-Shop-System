...
switch _mode do {
	case "initsystem":{
		//--- Init the system
		{
			_fnc = format ["TER_fnc_%1", _x];
			//uiNamespace setVariable [_fnc, call compile _fnc];
		}forEach ["shop","getItemValues"];
		//[] call compile preprocessFileLineNumbers "VASS\config.sqf";
...

		{
			...
			if (ctrltype _ctrlList == 102) then {
				...
				for "_row" from 0 to (lnbSize _ctrlList select 0) do {
					...
					_itemValues params ["", "_itemCost","_itemMax"];
					_symbol = [INF, str _amount] select (_amount > -1);
					...
					_ctrlList lnbSetText [[_row, _newColumn], format ["%1$", [TER_VASS_shopObject, _item, 1] call TER_fnc_getItemValues]];
				...
				for "_row" from 0 to (lbSize _ctrlList) do {
					...
					if (_item != "") then {
						...
						_amount = [TER_VASS_shopObject, _item, 2] call TER_fnc_getItemValues;
						_symbol = [INF, str _amount] select (_amount > -1);
						_ctrlList lbSettext [_row, format ["%1x %2", _symbol, _curText]];
						_ctrlList lbSetTextRight [_row,format ["%1$", [TER_VASS_shopObject, _item, 1] call TER_fnc_getItemValues]];
	...
	case "buttonCargo": {
		...
		_addAllowed = [_addCur < _addMax, true] select (_addMax < 0);
		...
		_value = {_x == _item} count _items;
		_text = [format ["%1|%2", _value, _addMax - _addCur - _mpCost], str _value] select (_addMax < 0);
			...
			if (_active) then {
				...
				if (
					...
					for "_l" from 0 to (lbsize _ctrlList - 1) do {
						...
						_text = [format ["%1|%2", _value, _itemMax - _addCur], str _value] select (_itemMax < 0);
		...
		switch _index do
		{
			...
			case IDC_RSCDISPLAYARSENAL_TAB_PRIMARYWEAPON: {
				...
				if (_isDifferentWeapon) then {
					...
					_loadedMags = primaryWeaponMagazine _center;
					if (count _loadedMags > 0) then {
						{
							if (_center canAdd _x) then {
								_center addItem _x;
							} else {
			...
			case IDC_RSCDISPLAYARSENAL_TAB_SECONDARYWEAPON: {
				...
				if (_isDifferentWeapon) then {
					...
					_loadedMags = secondaryWeaponMagazine _center;
					if (count _loadedMags > 0) then {
						{
							if (_center canAdd _x) then {
								_center addItem _x;
			...
			case IDC_RSCDISPLAYARSENAL_TAB_HANDGUN: {
				...
				if (_isDifferentWeapon) then {
					...
					_loadedMags = handgunMagazine _center;
					if (count _loadedMags > 0) then {
						{
							if (_center canAdd _x) then {
								_center addItem _x;
					...
					_cfgMuzzle = if (_x == "this") then {_cfgWeapon} else {_cfgWeapon >> _x};
					{
						...
						if (CONDITION(_virtualMagazineCargo)) then {
							...
							if (!(_mag in _magazines) && {getnumber (_cfgMag >> "scope") == 2 || getnumber (_cfgMag >> "scopeArsenal") == 2}) then {
								...
								_itemMax = [TER_VASS_shopObject, _mag, 2] call TER_fnc_getItemValues;
								_addMax = _itemMax - ([_mag] call _fncCurAdd);
								_text = [format ["%1|%2", _value, _addMax], str _value] select (_itemMax < 0);
								_lbAdd = _ctrlList lnbaddrow ["", _displayName, _text, format ["%1$", [TER_VASS_shopObject, _item, 1] call TER_fnc_getItemValues]];
		...
		if (
			...
			_compatibleItems = _item call bis_fnc_compatibleItems;
			{
				...
				if (_scope == 2 && CONDITION(_virtualItemCargo)) then {
					...
					_ctrlList lbSetTextRight [_lbAdd,format ["%1$", [TER_VASS_shopObject, _item, 1] call TER_fnc_getItemValues]];
